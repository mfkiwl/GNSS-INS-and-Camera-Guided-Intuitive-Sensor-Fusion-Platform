...
class Detect():
    ...
    def detect_lane(self):
        ...
        line = np.zeros((ll_seg_mask.shape[0], ll_seg_mask.shape[1], 3), dtype=np.uint8) # değişken ismi 'img' olunca hata alıyor
        line[ll_seg_mask==1] = [0, 255, 0]
        # line[::2, ::2, :] = 0

        line = cv2.resize(line, (self.cv_image.shape[1],self.cv_image.shape[0]))
        line = cv2.rotate(line, cv2.ROTATE_90_COUNTERCLOCKWISE)
        line = cv2.flip(line, 0)

        points = zip(list(np.where(line==255)[0]), list(np.where(line==255)[1]))
