...
#include <mst_object_localization/PointCloud2WithId.h>
  ...
  : m_node_handle(t_node_handle), m_action_client("cloud_painter/cloud_projector", true)
{
    ...
    m_clouds_publisher = m_node_handle.advertise<mst_object_localization::ArrayOfPointCloud2s>(
        "filtered_clouds", 1);
...
void CloudFilter::yoloTopicCallback(const yolov5_ros_msgs::BoundingBoxesConstPtr& t_yolo_boxes)
{
    ...
    mst_object_localization::ProjectedCloudGoal goal;
    goal.x_max = 0;
    goal.y_max = 0;
    ...
    mst_object_localization::ArrayOfPointCloud2s cloud_array;
    ...
    for (auto box : t_yolo_boxes->bounding_boxes)
    {
        ...
        mst_object_localization::PointCloud2WithId cloud_with_id;
        cloud_with_id.header.stamp = ros::Time::now();
        cloud_with_id.Class = box.Class;
