...
class Detection
{
  private:
    ...
    //! ActionServer.
    actionlib::SimpleActionServer<mst_object_localization::SignTrackerAction> m_action_server;

    /*!
     * Action server callback method for transmitting projected point cloud to CloudFilter.
     * @param goal action goal.
     */
    void actionServerCallback(const mst_object_localization::SignTrackerGoalConstPtr& goal);

  public:
    Detection(int argc, char** argv, ros::NodeHandle& t_node_handle);

    void pointCloudCallback(
        const mst_object_localization::ArrayOfPointCloud2sConstPtr& cloud_sign_array);
};
